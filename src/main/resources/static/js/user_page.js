$(document).ready(function () {/*функция которая выполнится как только загрузится index.html*/
    ...
    $.getJSON("/name_user", function (data) {/*выполнить запрос на енд-поинт '/rate'   data - десиарилизированный результат, который прилетит из RateController */
        $('#firstName').text(data.firstName);/*на место элемента с id-шником '#rate' в index.html подставляем текст UAH обьекта rates обьекта data*/
        $('#lastName').text(data.lastName);/*на место элемента с id-шником '#date' в index.html подставляем текст date обьекта data*/
    });
});